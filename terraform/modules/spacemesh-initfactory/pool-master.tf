...
module "eks-master" {
  ...
  basename = "${local.clusters.initfactory}-master"
  pool     = "master"
  taint    = false
  ...
  node_instance_type = var.initfactory_master_instance_type
  node_ebs_size      = var.initfactory_master_ebs_size
...
resource "aws_security_group_rule" "eks-master-coredns" {
  ...
  cidr_blocks = [var.initfactory_vpc_cidr]
}
...
# Inline IAM Policy: Allow cluster-autoscaler to call DescribeTemplateVersion
